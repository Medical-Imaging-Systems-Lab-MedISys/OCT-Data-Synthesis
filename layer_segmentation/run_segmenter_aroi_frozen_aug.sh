#!/bin/bash
#SBATCH --job-name=RETFound_AROI_Frozen_Aug
#SBATCH --output=logs/retfound_aroi_frozen_aug_%j.out
#SBATCH --error=logs/retfound_aroi_frozen_aug_%j.err
#SBATCH --nodes=1
...
export LOCAL_DATA_DIR="/tmp/AROI_frozen_aug_${SLURM_JOB_ID}"
...
# Execute Training
echo "Starting RETFound Frozen Backbone Segmentation Training on AROI (with Augmentations)..."
...
/data/vds/env_pt/bin/python train_aroi.py \
    ...
    --use_augmentations \
    --img_size 256 \
    --run_name "RETFound_AROI_Frozen_Aug_256"
