#!/bin/bash
#SBATCH --job-name=RETFound_OCTID_Finetune_Aug
#SBATCH --output=logs/retfound_octid_finetune_aug_%j.out
#SBATCH --error=logs/retfound_octid_finetune_aug_%j.err
#SBATCH --nodes=1
...
export LOCAL_DATA_DIR="/tmp/OCTID_finetune_aug_${SLURM_JOB_ID}"
...
# Execute Training
echo "Starting RETFound Finetune Segmentation Training on OCTID (with Augmentations)..."
...
/data/vds/env_pt/bin/python train_octid.py \
    ...
    --use_augmentations \
    --img_size 256 \
    --run_name "RETFound_OCTID_Finetune_Aug_256"
