...
def main():
    parser = argparse.ArgumentParser(description="Segment GOALS OCT Images using trained RETFound models")
    ...
    mlflow.set_experiment('GOALS_Segmentation')
    ...
    # 3. Load Hugging Face GOALS Dataset
    print("Loading GOALS dataset from Hugging Face...")
    dataset = load_dataset('ai4ophth/GOALS-oct-dataset', split='train')
    
    num_samples = len(dataset)
    print(f"Total dataset size: {num_samples} images.")
    ...
    with mlflow.start_run(run_name=args.run_name):
        ...
        mlflow.set_tag("dataset", "goals")
        
        with torch.no_grad():
            for idx in tqdm(range(num_samples), desc="Segmenting"):
                row = dataset[idx]
                img_pil = row['OCT_image'].convert('RGB')
                ...
                filename = f"goals_scan_{idx}.png"
        ...
        plt.tight_layout()
        grid_path = f"goals_eval_{args.run_name}_grid.png"
