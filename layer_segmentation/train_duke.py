...
from dataset_duke import DukeDMEDataset
...
def train():
    parser = argparse.ArgumentParser(
        description="Train RETFound Segmenter on Duke DME Chiu 2015 dataset")
    parser.add_argument('--data_dir', type=str, required=True,
                        help="Root Duke DME directory (parent of "
    ...
    parser.add_argument('--run_name', type=str,
                        default="RETFound_DukeDME_Finetune_Aug_256",
    ...
    # 1. Prepare Data
    train_dataset = DukeDMEDataset(
        data_dir=args.data_dir,
        split='train',
        img_size=args.img_size,
        use_augmentations=args.use_augmentations,
    )
    val_dataset = DukeDMEDataset(
        data_dir=args.data_dir,
        split='val',
        img_size=args.img_size,
    ...
    mlflow.set_experiment("DukeDME_Segmentation")
    ...
    with mlflow.start_run(run_name=args.run_name):
        ...
        mlflow.set_tag(
            ...
            "Dataset: Duke DME Chiu 2015 (10 subjects, 61 B-scans each). "
            "NUM_CLASSES=9: 0=vitreous, 1-7=retinal layers (8 boundaries), "
            "8=choroid. "
            "Layer boundaries from manualLayers1 converted to pixel masks "
            "per slice (convert_duke_masks.py). NaN columns skipped. "
            ...
            "80/20 subject-level split (8 train, 2 val, seed=42)."
        )
