#!/bin/bash
#SBATCH --job-name=RETFound_NR206_Frozen_WM_Removed_Aug
#SBATCH --output=logs/retfound_nr206_frozen_wm_removed_aug_%j.out
#SBATCH --error=logs/retfound_nr206_frozen_wm_removed_aug_%j.err
#SBATCH --nodes=1
...
export LOCAL_DATA_DIR="/tmp/NR206_frozen_wm_removed_aug_${SLURM_JOB_ID}"
...
# Execute Training
echo "Starting Frozen RETFound Segmentation Training on NR206 (WM removed, with Augmentations)..."
...
/data/vds/env_pt/bin/python train_nr206.py \
    ...
    --freeze_backbone \
    --use_augmentations \
    --run_name "RETFound_NR206_Frozen_WM_Removed_Aug"
