...
from dataset_manual import ManualDelineationDataset

# --- Hyperparameters ---
BATCH_SIZE = 16
EPOCHS = 50
LEARNING_RATE = 1e-4
NUM_CLASSES = 12  # 0=vitreous, 1-10=retinal layers (11 boundaries), 11=choroid
...
def train():
    parser = argparse.ArgumentParser(
        description="Train RETFound Segmenter on OCT Manual Delineations 2018")
    parser.add_argument('--data_dir', type=str, required=True,
                        help="Root Manual Delineations directory (parent of "
    ...
    parser.add_argument('--run_name', type=str,
                        default="RETFound_ManualDelineations_Finetune_Aug_256",
    ...
    # 1. Prepare Data
    train_dataset = ManualDelineationDataset(
        data_dir=args.data_dir,
        split='train',
        img_size=args.img_size,
        use_augmentations=args.use_augmentations,
    )
    val_dataset = ManualDelineationDataset(
        data_dir=args.data_dir,
        split='val',
        img_size=args.img_size,
    ...
    mlflow.set_experiment("ManualDelineations_Segmentation")
    ...
    with mlflow.start_run(run_name=args.run_name):
        ...
        mlflow.set_tag(
            ...
            "Dataset: OCT Manual Delineations 2018 (35 Spectralis .vol "
            "files, 49 B-scans each). "
            "NUM_CLASSES=12: 0=vitreous, 1-10=retinal layers (11 "
            "boundaries), 11=choroid/sclera. "
            "Boundaries from control_pts (49×11) interpolated with scipy "
            "across image width per B-scan (convert_manual_masks.py). "
            ...
            "Preprocessing: B-scan images read with heyexReader, converted "
            "to 3-channel by repetition, normalised with ImageNet mean/std. "
            "80/20 subject-level split (seed=42)."
        )
