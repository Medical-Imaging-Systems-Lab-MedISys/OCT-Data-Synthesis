#!/bin/bash
#SBATCH --job-name=RETFound_NR206_Finetune_NoWM_Aug
#SBATCH --output=logs/retfound_nr206_finetune_nw_aug_%j.out
#SBATCH --error=logs/retfound_nr206_finetune_nw_aug_%j.err
#SBATCH --nodes=1
...
export LOCAL_DATA_DIR="/tmp/NR206_finetune_nw_aug_${SLURM_JOB_ID}"
...
# Execute Training
echo "Starting RETFound Finetune Segmentation Training on NR206 (No WM removal, with Augmentations)..."
...
/data/vds/env_pt/bin/python train_nr206.py \
    ...
    --use_augmentations \
    --run_name "RETFound_NR206_Finetune_NoWM_Aug"
