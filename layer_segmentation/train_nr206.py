...
EPOCHS = 50
LEARNING_RATE = 1e-4
NUM_CLASSES = 9  # Background + 8 layers
...
def train():
    parser = argparse.ArgumentParser(description="Train RETFound Segmenter on NR206")
    parser.add_argument('--data_dir', type=str, required=True, help="Path to the root of the NR206 dataset")
    ...
    parser.add_argument('--run_name', type=str, default="RETFound_NR206", help="MLflow run name")
    parser.add_argument('--use_augmentations', action='store_true', help="Apply Albumentations data augmentation")
    parser.add_argument('--no_watermark_removal', action='store_true', help="Disable watermark removal (covering) on images")
    ...
    # 1. Prepare Data
    # In NR206: train images are in "train", train masks/labels are in "train_labels"
    # test images are in "test", test masks/labels are in "test_labels"
    train_dataset = NR206Dataset(
        image_dir=f'{args.data_dir}/train', 
        mask_dir=f'{args.data_dir}/train_labels',
        img_size=args.img_size,
        remove_watermark=not args.no_watermark_removal,
        use_augmentations=args.use_augmentations
    )
    val_dataset = NR206Dataset(
        image_dir=f'{args.data_dir}/test', 
        mask_dir=f'{args.data_dir}/test_labels',
        img_size=args.img_size,
        remove_watermark=not args.no_watermark_removal,
    ...
    mlflow.set_experiment("NR206_Segmentation")
    ...
    with mlflow.start_run(run_name=args.run_name):
        ...
        mlflow.set_tag("mlflow.note.content", "RETFound layer segmentation on NR206 dataset.")
