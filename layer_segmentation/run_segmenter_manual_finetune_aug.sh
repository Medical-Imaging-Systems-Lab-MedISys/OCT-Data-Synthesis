#!/bin/bash
#SBATCH --job-name=RETFound_ManualDelineations_Finetune_Aug
#SBATCH --output=logs/retfound_manual_finetune_aug_%j.out
#SBATCH --error=logs/retfound_manual_finetune_aug_%j.err
#SBATCH --nodes=1
...
export LOCAL_DATA_DIR="/tmp/ManualDelineations_finetune_aug_${SLURM_JOB_ID}"
...
# Execute Training
echo "Starting RETFound Finetune Segmentation Training on Manual Delineations (with Augmentations)..."
...
/data/vds/env_pt/bin/python train_manual.py \
    ...
    --use_augmentations \
    --img_size 256 \
    --run_name "RETFound_ManualDelineations_Finetune_Aug_256"
