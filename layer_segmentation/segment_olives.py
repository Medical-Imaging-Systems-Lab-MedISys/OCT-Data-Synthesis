...
def main():
    parser = argparse.ArgumentParser(description="Segment OLIVES OCT Images using trained RETFound models")
    ...
    parser.add_argument('--limit', type=int, default=1000, help="Maximum number of images to segment (-1 to segment all)")
    ...
    mlflow.set_experiment('OLIVES_Segmentation')
    ...
    # 3. Load Hugging Face OLIVES Dataset
    print("Loading OLIVES dataset from Hugging Face...")
    dataset = load_dataset('gOLIVES/OLIVES_Dataset', 'disease_classification', split='train')
    
    total_images = len(dataset)
    num_samples = total_images if args.limit == -1 else min(args.limit, total_images)
    print(f"Total dataset size: {total_images}. Segmenting first {num_samples} images.")
    ...
    with mlflow.start_run(run_name=args.run_name):
        ...
        mlflow.set_tag("dataset", "olives")
        
        with torch.no_grad():
            for idx in range(num_samples):
                row = dataset[idx]
                img_pil = row['Image'].convert('RGB')
                ...
                eye_id = row['Eye_ID']
                scan_idx = row['Scan (n/49)']
                filename = f"Patient_{patient_id}_Eye_{eye_id}_Scan_{scan_idx}_{idx}.png"
                ...
                if idx in grid_indices:
                    ...
                    sample_grids.append((img_np, pred_color, filename))
        ...
        plt.tight_layout()
        grid_path = f"olives_eval_{args.run_name}_grid.png"
