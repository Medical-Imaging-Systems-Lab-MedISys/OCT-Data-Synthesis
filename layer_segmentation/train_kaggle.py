...
from dataset_kaggle import KaggleRetinalDataset

# --- Hyperparameters ---
BATCH_SIZE = 16
EPOCHS = 50
LEARNING_RATE = 1e-4
NUM_CLASSES = 8   # grayscale values [0,36,72,109,145,182,218,255] → 0-7
...
def train():
    parser = argparse.ArgumentParser(
        description="Train RETFound Segmenter on Kaggle Retinal dataset")
    parser.add_argument('--data_dir', type=str, required=True,
                        help="Root of Kaggle Retinal Segmentation Dataset "
                             "(parent of resized_images/ and "
    ...
    parser.add_argument('--run_name', type=str,
                        default="RETFound_Kaggle_Finetune_Aug_256",
    ...
    image_dir = os.path.join(args.data_dir, 'resized_images')
    mask_dir = os.path.join(args.data_dir, 'resized_labeledimages')

    # 1. Prepare Data
    train_dataset = KaggleRetinalDataset(
        image_dir=image_dir,
        mask_dir=mask_dir,
        split='train',
    ...
    val_dataset = KaggleRetinalDataset(
        image_dir=image_dir,
        mask_dir=mask_dir,
        split='val',
        img_size=args.img_size,
    ...
    mlflow.set_experiment("Kaggle_Retinal_Segmentation")
    ...
    with mlflow.start_run(run_name=args.run_name):
        ...
        mlflow.set_tag(
            ...
            "Dataset: Kaggle Retinal Segmentation (8 retinal layers). "
            "NUM_CLASSES=8: grayscale mask values [0,36,72,109,145,182,218,"
            "255] mapped to class IDs 0-7 via LUT. "
            ...
            "Preprocessing: grayscale images converted to 3-channel by "
            "repetition, normalised with ImageNet mean/std. "
            "80/20 file-level train/val split (sorted, seed=42)."
        )
