...
class PairedOCTDataset(Dataset):
    def __init__(self, labels_dir, real_dir, img_size, transform=None, normalization='minmax', is_train=True):
        ...
        for fname in self.filenames:
            ...
            real_np_squashed = cv2.resize(real_np, (self.img_size, self.img_size), interpolation=cv2.INTER_LINEAR)
            mask_squashed = cv2.resize(mask_bgra, (self.img_size, self.img_size), interpolation=cv2.INTER_LINEAR)
            
            # Crop and pad real image with curved boundary AFTER resizing
            real_np_padded = crop_and_pad_curved(real_np_squashed, mask_squashed)
            
            real_img_resized = Image.fromarray(real_np_padded, mode='L')
            ...
            # Crop and pad synthetic image AFTER resizing
            synth_np_squashed = cv2.resize(synth_np, (self.img_size, self.img_size), interpolation=cv2.INTER_LINEAR)
