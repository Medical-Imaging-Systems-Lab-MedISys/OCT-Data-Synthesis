...
def generate_samples(model, x0, num_steps=50):
    ...
    for i in range(num_steps):
        ...
        # Predict velocity field. (Diffusers UNet returns a tuple/class)
...
class NR206DynamicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1. Load the Ground Truth Real OCT (x1)
        # Using cv2.imread for fast disk I/O, output shape is (H, W)
        ...
        if mask_bgra is None:
            raise ValueError(f"Could not read mask image: {lbl_path}")
            
        # If read with only 3 channels (BGR), pad with an alpha channel to make it BGRA
        # This replicates the safety check from your prepare_synthetic_dataset script
        ...
        # 4. Normalize to [-1.0, 1.0] for Flow Matching model stability
        x0 = (x0_img.astype(np.float32) / 127.5) - 1.0
        x1 = (x1_img.astype(np.float32) / 127.5) - 1.0
        
        # 5. Convert to PyTorch Tensors with channel dimension -> Shape: [1, H, W]
...
def train():
    ...
    train_dataset = NR206DynamicDataset(
        ...
        real_dir=os.path.join(local_data_dir, "train"),
        min_gamma=0.5,
        max_gamma=1.5
    )

    # Crucial: Set num_workers=6 to parallelize the runtime synthesis
                ...
                with torch.no_grad():
                    for batch_idx, (val_x0, val_x1) in enumerate(val_loader):
                        ...
                        if logged_images_count < CONFIG["num_val_images"]:
                            # Generate synthesis by solving the ODE
