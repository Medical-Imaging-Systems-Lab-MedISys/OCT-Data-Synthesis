...
CONFIG = {
    ...
    "run_name": f"cfm_no_watermark_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}",
...
def train():
    ...
    with mlflow.start_run(run_name=f"cfm_no_watermark_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"):
        # Log all configuration parameters
        mlflow.log_params(CONFIG)
        
        # Add run description/notes
        mlflow.set_tag("mlflow.note.content", "CFM training run with dynamic watermark removal applied to real OCT images (bottom-left corner flipped) with L2 Loss.")
